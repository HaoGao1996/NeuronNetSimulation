...
sys.path.append("C:/Users/lenovo/Desktop/assimilation")
import os
import time
import brain_block.random_initialize as ri
import multiprocessing as mul

with open('C:/Users/lenovo/Desktop/assimilation/1000_Ca_signal_500ms.pickle','rb+') as f:
    Y=pickle.load(f)
with open('C:/Users/lenovo/Desktop/assimilation/1000_Ca_signal_500ms2.pickle','rb+') as f:
    Ct=pickle.load(f)
with open('C:/Users/lenovo/Desktop/assimilation/1000_Ca_signal_property.pickle','rb+') as f:
...
def cell_KF(arg):
    cells, cell_index = arg
    mu_cell = cells.mean(axis=0).reshape(-1,1)
    cells=cells.reshape(samples_size,-1).T#注意是否存在乱序
    cells+=torch.tensor(np.random.normal(0,0.02,[cells_state_size,samples_size]),device=device)#draw
    ...
    K_kf = half_cov.T/S
    samples_obs = torch.normal(Y[t,cell_index],gama,[samples_size],device=device)#观测量的分析
    gain=torch.matmul(K_kf,(samples_obs - cells[-1,:]).reshape(1,-1)).reshape([state_size, K, samples_size]).permute(2,1,0)
...
gama = torch.tensor(0.1**2,device=device)
...
if __name__ == '__main__':
    for t in range(T_size):
        ...
        if t%obs_interval==0:#观测节点,Kalman update
            all_gain = 0
            pool = mul.Pool(length)
            for cell_index_ in range(K//length):
                add_gains = pool.map(cell_KF,[(all_cell_states,cell_index) for cell_index in range(cell_index_,cell_index_+length)])
                all_gain+=sum(add_gains)
