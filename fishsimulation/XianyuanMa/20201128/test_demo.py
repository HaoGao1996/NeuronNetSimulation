...
class TestBlock(unittest.TestCase):
    ...
    def test_compare_via_dist_noise(self):
        ...
        path = '/home1/bychen/spliking_nn_for_brain_simulation/dti_single_200k_new'
        dist_block = BlockWrapper('192.168.2.92:50051', os.path.join(path, 'single'), 0.01, 1.)

        print(dist_block.subblk_id)
        '''
        ...
        '''
        sample_idx = np.arange(dist_block.total_neurons).astype(np.uint32)
        run_number = 4000

        #cpu_path  = "./single_small/single"
        cpu_path  = os.path.join(path, 'single')
        property, w_uij = ri.connect_for_block(cpu_path)

        gui = property[0, 10:14]
        #new_p = np.stack([np.random.exponential(g, property.shape[0]) for g in gui], axis=-1).astype(np.float32)
        #new_p =  0.5 * np.random.randn(property.shape[0], 1).astype(np.float32)

        #property_idx = np.stack(np.meshgrid(sample_idx.astype(np.uint32), np.arange(2, 3, dtype=np.uint32), indexing='ij'), axis=-1)
        #property_idx = property_idx.reshape([-1, 2])

        dist_block.set_samples(sample_idx)
        #dist_block.update_property(property_idx, new_p.reshape([-1]))
        ...
        sample_noise = sample_log_with_noise - sample_log.astype(sample_log_with_noise.dtype)
        np.save('noise', sample_noise)
        np.save('vi', sample_vi)
        ...
        with mock.patch('torch.Tensor.__or__',  _or) as n:
            ...
            #property[:, 2:3] = torch.from_numpy(new_p)
            #cpu_block.update_property(property)
    ...
    def _test_speed(self):
        from cuda.python.dist_blockwrapper import BlockWrapper
        #path = '/home1/bychen/spliking_nn_for_brain_simulation/dti_30_600m/single'
        path = 'C:/Users/lenovo/Desktop/spliking_nn_for_brain_simulation-master/single_small_test/single'
        length = 800
        ...
        block = BlockWrapper('192.168.2.91:50051', path, 0, 1.,)
        block.shutdown()
