...
if __name__ == '__main__':
    ...
    for t in range(T_size):
        if t%10 == 0:
            print("t:{}".format(t))
        compute = []
        B_index_list = list(range(samples_size))
        for sample_index in B_index_list:
            com = delayed(update)(sample_index)
            compute.append(com)
        total_compute = delayed()(compute)
    ...
    print("a: {},b: {},yj {}".format(sigmoid_func(all_cell_states[:,:,2].mean(dim=[0,1]).to("cpu"), a_low_bound, a_upper_bound),\
                                     sigmoid_func(all_cell_states[:,:,3].mean(dim=[0,1]).to("cpu"), b_low_bound, b_upper_bound),\
                                     sigmoid_func(all_cell_states[:,:,4].mean(dim=[0,1]).to("cpu"), yj_low_bound, yj_upper_bound)))
    plt.figure(1)
